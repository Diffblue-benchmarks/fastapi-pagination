"""Tests for fastapi_pagination.ext.orm (apaginate, paginate)."""
...
# under test. The `orm` package is not installed in this environment.
# ============================================================================


class _MockQuerySet:
    """Mock for orm.models.QuerySet."""
...
_mock_orm_models = MagicMock()
_mock_orm_models.QuerySet = _MockQuerySet

_MOCK_MODULES = {
    "orm": MagicMock(),
    "orm.models": _mock_orm_models,
}
...
class TestPaginate:
    ...
    async def test_paginate_delegates_to_apaginate(self) -> None:
        ...
        with patch(
            "fastapi_pagination.ext.orm.apaginate",
...
class TestApaginate:
    @pytest.mark.asyncio
    async def test_apaginate_returns_run_async_flow_result(self) -> None:
        mock_query = MagicMock(spec=_MockQuerySet)
        mock_result = MagicMock()

        with patch(
            "fastapi_pagination.ext.orm.run_async_flow",
            new=AsyncMock(return_value=mock_result),
        ) as mock_run:
    ...
    async def test_apaginate_calls_generic_flow(self) -> None:
        ...
        with (
            patch(
                "fastapi_pagination.ext.orm.generic_flow",
