...
def test_create_paginate_query_with_limit_and_offset():
    raw = RawParams(limit=10, offset=5)
    result = create_paginate_query_from_text("SELECT * FROM foo", raw)
    assert result == "SELECT * FROM foo  LIMIT 10 OFFSET 5"
...
def test_create_paginate_query_with_offset_only():
    ...
    result = create_paginate_query_from_text("SELECT * FROM foo", raw)
    assert result == "SELECT * FROM foo  OFFSET 5"


def test_create_paginate_query_without_limit_or_offset():
...
def test_create_paginate_query_with_abstract_params():
    ...
    result = create_paginate_query_from_text("SELECT * FROM bar", params)
    assert "LIMIT 50" in result
    assert "OFFSET 0" in result


def test_create_count_query_from_text():
    query = "SELECT * FROM foo WHERE active = true"
